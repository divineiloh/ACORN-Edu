...
def test_profiles_diverge():
    ...
    bytes_diff = abs(wifi_data["bytes_mean_kb"].iloc[0] - cellular_data["bytes_mean_kb"].iloc[0])
    hit_rate_diff = abs(wifi_data["hit_rate_mean_percent"].iloc[0] - cellular_data["hit_rate_mean_percent"].iloc[0])
    
    assert bytes_diff > 1.0 or hit_rate_diff > 1.0, f"Profiles too similar: bytes_diff={bytes_diff}, hit_rate_diff={hit_rate_diff}"
...
def test_oaec_no_fp_fn():
    ...
    assert tampered_row["predicted_valid"] == 0, f"False negatives detected: {tampered_row['predicted_valid']}"
    assert valid_row["predicted_tampered"] == 0, f"False positives detected: {valid_row['predicted_tampered']}"
...
def test_scheduler_names():
    ...
    assert not any("sabi" in s.lower() for s in schedulers), "Found Sabi references in scheduler names"
    
    # Check ablation config names
    ablation_configs = set(abl["ablation_config"].unique())
    assert not any("sabi" in config.lower() for config in ablation_configs), "Found Sabi references in ablation configs"
    ...
    for df in [net, abl]:
        for col in ["bytes_mean_kb", "hit_rate_mean_percent"]:
            ...
            if lower_col in df.columns and upper_col in df.columns:
                assert (df[lower_col] <= df[mean_col]).all(), f"CI lower > mean for {col}"
                assert (df[mean_col] <= df[upper_col]).all(), f"CI upper < mean for {col}"