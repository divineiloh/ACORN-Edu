...
def test_oaec_perfect_detection():
    ...
    # Calculate metrics
    tp = sum(not result for result in tampered_results)  # Correctly identified as tampered
    tn = sum(valid_results)  # Correctly identified as valid
...
def test_ablation_weight_application():
    ...
    assert hit_rate_varies or bytes_varies, "Ablation weights not producing different results"
...
def test_network_profile_sensitivity():
    ...
    # Run simulations
    wifi_result = wifi_sim.run_simulation_pass('AcornScheduler', weights=(0.5, 0.2, 0.2, 0.1))
    cellular_result = cellular_sim.run_simulation_pass('AcornScheduler', weights=(0.5, 0.2, 0.2, 0.1))
    ...
    assert hit_rate_diff > 1.0 or bytes_diff > 1000, f"Network profiles too similar: hit_rate_diff={hit_rate_diff}, bytes_diff={bytes_diff}"
...
def test_priority_calculation():
    ...
    # Test asset
    asset = {"id": "Test", "size": 100 * 1024 * 1024, "deadline_hours": 10, "reuse_score": 0.5}
    ...
    assert abs(priority1 - priority2) > 0.01, "Priority calculation not sensitive to weights"
