...
class MockSigner:
    ...
    def verify(self, data, signature, public_key):
        ...
        if public_key != self.public_key:
            return False
        expected_signature = hashlib.sha256(f"{data}|{self.private_key}".encode()).hexdigest()
    ...
    def _calculate_hit_rate(self, download_log, assets):
        ...
        for asset in assets:
            bytes_at_deadline = sum(
                size for aid, hour, size in download_log
        ...
        for hour, (speed, net_type) in enumerate(self.network_trace):
            ...
            if scheduler_type == 'AcornScheduler':
                ...
                while bw > self.avg_chunk_size:
                    ...
                    for asset in self.assets:
                        if any(c not in downloaded_chunks for c in self.chunk_db[asset['id']]):
                            priority = self._calculate_priority(asset, hour, weights, net_factor)
                            if priority > max_priority:
                                max_priority, best_asset = priority, asset
                    if not best_asset:
                        break

                    chunk_to_download = next((c for c in self.chunk_db[best_asset['id']] if c not in downloaded_chunks), None)
                    if chunk_to_download and chunk_to_download not in downloaded_chunks:
                        ...
                        download_log.append((best_asset['id'], hour, self.avg_chunk_size))
...
class OAEC_Simulator:
    ...
    def _tamper_chain(self, chain):
        ...
        if act == "del" and len(t_chain) > 1:
            ...
            t_chain[2]['payload'] = json.dumps({"event": "cheat", "ts": self.t0 + 2}, separators=(",", ":"), sort_keys=True)
...
class CAG_Simulator:
    ...
    def run_simulation(self):
        ...
        tp, fp, fn = len(validated & self.gt), len(validated - self.gt), len(self.gt - validated)
        prec = tp / (tp + fp) if (tp + fp) > 0 else 0
        rec = tp / (tp + fn) if (tp + fn) > 0 else 0
        f1 = 2 * prec * rec / (prec + rec) if (prec + rec) > 0 else 0
        print(f"\nCompiler Metrics: Precision: {prec:.2f}, Recall: {rec:.2f}, F1: {f1:.2f}")
        write_csv("cag_metrics.csv", ["precision", "recall", "f1_score"], [[prec, rec, f1]])
        headers = ["id", "status", "content"]
        rows = []
        for oid in self.std:
            status = "Validated" if oid in validated else ("Covered" if any(m['c'] == oid for m in self.lib.values()) else "GAP")
...
def calculate_cis(data, confidence=0.95):
    ...
    n = len(a)
    if n < 2:
        return np.mean(a) if n > 0 else 0, np.mean(a) if n > 0 else 0, np.mean(a) if n > 0 else 0
...
if __name__ == "__main__":
    ...
    for run in range(NUM_SIMULATION_RUNS):
        ...
        for profile in network_profiles:
            ...
            for scheduler in schedulers_to_test:
                ...
                network_results.append([profile, scheduler, res['bytes'], res['hit_rate']])
    ...
    for profile in network_profiles:
        for scheduler in schedulers_to_test:
            ...
            network_agg[key] = {
                'bytes_mean': bytes_m, 'bytes_ci_lower': bytes_l, 'bytes_ci_upper': bytes_u,
                'hit_rate_mean': hr_m, 'hit_rate_ci_lower': hr_l, 'hit_rate_ci_upper': hr_u
            }
    ...
    run_metadata = {
        "seed_base": RANDOM_SEED_BASE, "num_runs": NUM_SIMULATION_RUNS,
